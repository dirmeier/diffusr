...
`diffusr` implements algorithms for network diffusion such as *Markov random walks with restarts*, *nearest neighbors*, *insulated heat diffusion* and *Laplacian heat diffusion*. Network diffusion has been studied extensively in bioinformatics, e.g. in the field of cancer gene prioritization. In the first versions of `diffusr` the four methods above are implemented, several others will follow in future patches.
